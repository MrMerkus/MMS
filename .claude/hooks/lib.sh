...
# Tek kök mimarisi: asistan kökten açılırsa CLAUDE_PROJECT_DIR kök olur, oysa hafıza
# bir alt klasördedir. Refleks katmanını (🔮 zihin) arayarak gerçek hafıza kökünü bul.
# Kök hiçbir şey barındırmaz; barındırsaydı konseyin "kökte .git olmaz" koşulu bozulurdu.
if [ ! -d "$BEYIN_PROJECT_DIR/🔮 zihin" ]; then
  # Aday yalnızca 'zihin' klasörü değil, CANLI vault işaretleri de taşımalı:
  # kendi .git deposu ve .obsidian yapılandırması. Aksi halde yanına konan bir
  # yedek kopyası ("AAA-eski-yedek/") alfabetik olarak önce gelip seçiliyor ve
  # bütün hafıza yazımı ölü kopyaya gidiyordu (denetimde deneyle doğrulandı).
  BEYIN_BULUNAN=""
  BEYIN_ADAY_SAYI=0
  for BEYIN_ADAY in "$BEYIN_PROJECT_DIR"/*/; do
    [ -d "$BEYIN_ADAY/🔮 zihin" ] || continue
    [ -d "$BEYIN_ADAY/.git" ] || continue
    [ -d "$BEYIN_ADAY/.obsidian" ] || continue
    BEYIN_ADAY_SAYI=$((BEYIN_ADAY_SAYI + 1))
    [ -z "$BEYIN_BULUNAN" ] && BEYIN_BULUNAN=${BEYIN_ADAY%/}
  done
  if [ -n "$BEYIN_BULUNAN" ]; then
    BEYIN_PROJECT_DIR=$BEYIN_BULUNAN
    # Birden fazla canlı aday belirsizliktir; sessizce birini seçmek yerine iz bırak.
    if [ "$BEYIN_ADAY_SAYI" -gt 1 ]; then
      mkdir -p "$BEYIN_PROJECT_DIR/.claude/scripts/.state" 2>/dev/null || :
      printf '%s\n' "$BEYIN_ADAY_SAYI aday vault bulundu, ilki seçildi: $BEYIN_PROJECT_DIR" \
        > "$BEYIN_PROJECT_DIR/.claude/scripts/.state/vault-belirsiz" 2>/dev/null || :
    fi
  fi
fi
...
beyin_emit() {
  ...
  case "$BEYIN_EVENT" in
    SessionStart|UserPromptSubmit|SessionEnd|PreCompact|PostToolUse) ;;
    *) return 0 ;;
  esac
